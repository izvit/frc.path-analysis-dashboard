...
@app.callback(
    Output(component_id='event_select', component_property='options'),
    Input(component_id='game-event-table', component_property='data'),
)
def event_select(table_data):
   pass
...
def get_match_data(team, match):
    df=pd.read_sql_query(f"SELECT AutoEventList FROM match WHERE Team={team} and Match={match}", con=db.engine)
    df = df.reset_index(drop=True)
    result = [json.loads(x) for x in df['AutoEventList']]
    if len(result)>0:
        ...
        dbc.Col([
            ...
            dash_table.DataTable(
                ...
                columns=[
                    ...
                    dict( id='noteId', name='Normalize Y Position' , type='numeric' ),
                ],
                style_cell={
                    "fontFamily": "Ubuntu", 
                    "fontSize": "12px", 
